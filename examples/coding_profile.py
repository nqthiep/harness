...
class CodingProfile:
    ...
    def apply(self, agent: Agent) -> Agent:
        ...
        # ONE approver, decided before either agent is built. The profile supplies a
        # terminal prompt when the caller brought none, and the reader has to get the
        # SAME one: computing it separately meant the reader took `agent.approve`
        # (None) while `built` took the fallback, so the parent had a human in the loop
        # and its own subagent did not — which is exactly what G-15/H-5 refuses, and it
        # refused this profile.
        ...
        reader = Agent(
            ...
            safety=agent.safety,
            # ...and `approve=` for the same reason, one axis over (G-15/H-5). A
            # subagent runs its OWN `atry_run` with its OWN `approve=`, never the
            # parent's, and `PolicyEngine.resolve()` auto-ALLOWs a surviving ASK when
            # there is no callback — so a reader built without one, inside a parent that
            # has a real human in the loop, would silently self-approve exactly the
            # decisions that human is there to make. `_check_subagent_safety` refuses
            # the construction now; this is the profile holding up its end.
            approve=approve,
