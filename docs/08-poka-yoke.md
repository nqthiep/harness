...
| 61 | **A subsystem no default configuration can reach**, documented as if it were active | P-10 / AC-33: reachable from the defaults, or the docs say it is not. *Context management: `max_result_tokens` × `steps` = 80 000 against a 120 000 threshold* | CI |
| 64 | **A test-suite exclusion that silently narrows what is verified** — `if "Agent(" in block: continue`, written to skip blocks needing a provider, skipped the tutorial's headline example while reporting a pass | Exclusions stub the dependency and execute, rather than skipping. Coverage of a document asserts a *count*, so a silently narrowed scan fails | CI |
