...
| R-24 | **Approval được nhầm là isolation** | **4** | **5** | **20** | **Đã mitigate (M7).** Nghiên cứu hợp nhất nói thẳng điều này về Cline, và harness từng mắc đúng lỗi đó: §01.5 xếp sandbox vào non-goal, nên lớp Isolation — một trong 8 lớp Poka-Yoke — hoàn toàn trống. M7 đóng phần thư viện làm được: workspace root (T-7.1), egress deny mặc định (T-7.2), seam `Sandbox` với `InProcess`/`Subprocess` (T-7.3), secret không vào tiến trình con (T-7.4). Container thật (Docker/Firecracker) vẫn là việc của người vận hành — `Sandbox` chỉ là chỗ cắm, ADR-047 nói thẳng. | [§17](17-research-alignment.md) M7 |
| R-25 | **Retry lặp lại một side effect đã xảy ra** | 4 | 5 | 20 | **Đã mitigate một phần (M6, M9).** Anti-pattern 3 của nghiên cứu, và OWASP xếp duplicate-action thành rủi ro riêng. `execute_once`/`idempotency_key()` (T-6.1) xây xong, có caller thật ở MỨC RUN qua `Idempotency-Key` header (T-9.2/`harness.server`) — một client timeout rồi retry request KHỞI ĐỘNG RUN không tạo run thứ hai. Chưa đóng ở MỨC MỘT LỜI GỌI TOOL bên trong một run đang chạy — `Dispatcher._invoke` chưa gắn `execute_once` (N-8, `design/07-risks-and-open-issues.md §3`). | [§17](17-research-alignment.md) M6, M9 |
| R-26 | **Mở MCP trước khi có isolation** | 3 | 5 | 15 | **Đã mitigate (M7 trước M9, đúng thứ tự dự tính).** MCP mang cả một hệ sinh thái tool bên thứ ba vào, và nghiên cứu cảnh báo protocol **không phải** security boundary (anti-pattern 5). `harness.mcp` (T-9.1) landing sau M7: tool từ MCP không khai hint mặc định là `DANGER` (server chưa `trusted`) — thắt hơn cả `external`, không phải nới ra thành `read`. | [§17](17-research-alignment.md) M9 |
