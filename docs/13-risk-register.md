...
| R-06 | **`run.py` accretes special cases** and stops being auditable | 4 | 3 | 12 | **Now materializing.** M0 left it at 175 code lines; M1–M3 took it to **241 against the 250 ceiling** — 96 % consumed, with M4 (subagents) still to come. IDL-13 says an overrun is a design signal, not something to refactor around, so the next addition that would cross it must be raised rather than absorbed. Likelihood raised 3→4. | IDL-13 |
