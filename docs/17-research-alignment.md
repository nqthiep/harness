...
| Reliability | 12% | 3/5 → **5/5** | 12.0 | Cancellation không còn bị nuốt (T-6.2), retry theo effect class (T-6.3), chaos harness tìm+sửa 2 lỗi thật N-2/N-4 (T-6.4). **Cập nhật sau ADR-058/064:** `ToolCall.idempotency_key` nay gắn ở cả ba điểm dispatch (`tests/test_roadmap.py::S-01` xanh), và backend LangGraph nay TIÊU THỤ nó thật qua `execute_once` cho mọi lời gọi `write`/`danger` khi có `build_agent(idempotency_store=...)` — không chỉ tồn tại mà không có caller nữa. N-1 (LangGraph không có timeout per-tool) vẫn CHƯA sửa, ngoài phạm vi dòng điểm này (per-call timeout, không phải idempotency/retry). |
...
| Observability | 10% | 3/5 → **4/5** | 8.0 | Canonical event adapter (T-9.3) đóng S-07 (`tests/test_roadmap.py` không track S-07 riêng, nhưng `to_canonical_json` dùng chung SSE+transcript, xem ADR-055). Service API (T-9.2) mở thêm một transport thật. **Cập nhật sau ADR-058/059:** S-02 (`ApprovalRecord` export top-level), S-03 (`RunContext.principal`/`.tenant_id`), S-14 (backpressure `deque(maxlen=...)`) đều **ĐÃ ĐÓNG** — ba trong bốn khoảng trống liệt kê lúc chấm ban đầu không còn đúng. **Còn mở, giữ ở 4/5 vì việc này:** N-6 (`model.response` vẫn thiếu `usage`/`latency_ms` so với `docs/05` tự hứa — chưa sửa). N-5 (retry cấp provider) thuộc Reliability hơn là Observability, không tính vào dòng này nữa. |
...
|---|---|---|---|
| S-01 | **Idempotency key trên mỗi tool call** | Anti-pattern 3; OWASP duplicate-action | **XONG (ADR-058).** `ToolCall.idempotency_key` (`f"{run_id}:{call_id}"`, `idempotency.py`, T-6.1) nay được gắn ở cả ba điểm dispatch thật (`dispatch.py`, `lg/runtime.py`'s `policy_gate`/`approval_gate`/`_regate`). `tests/test_roadmap.py::S-01` xanh. **Nay đã được TIÊU THỤ, không chỉ gắn kèm:** backend LangGraph cho mọi lời gọi `write`/`danger` đi qua `execute_once` dưới khoá này khi có `build_agent(idempotency_store=...)` — một thread hồi phục sau sự cố giữa node `tools` phát lại kết quả thay vì chạy lại lời gọi (ADR-064, `tests/test_idempotency_wired.py`). Vòng lặp classic vẫn không, và `idempotency.py` nói rõ vì sao: `run_id` sinh mới mỗi `atry_run()` nên khoá không bắc qua được một lần resume. |
