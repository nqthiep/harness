...
| `run_golden_set(agent, cases, total_budget=)` | Runs each `GoldenCase`, checks its `Trajectory` if it has one, reports `pass_rate` with a Wilson-scored CI (reusing `cost_per_success`'s interval math, §10.5.5) plus total cost and tokens. `total_budget=` (G-14, design/review-architect.md) is an optional AGGREGATE ceiling across the whole set, independent of any one case's own `agent.budget` — omit it and every case runs exactly as before; pass a `Budget`/string and the sweep stops early, `GoldenReport.budget_exhausted=True`, once the shared ledger runs out. |
| `benchmark(run_fn, n=, concurrency=, total_budget=, cost_of=)` | `p50`/`p95` latency, `throughput_per_s` at real bounded concurrency (an `asyncio.Semaphore`, same shape tool fan-out already uses), `cold_start_ms` separated from steady-state `warm_p50_ms`. `total_budget=`/`cost_of=` (H-3, design/review-architect-round3.md) are an optional aggregate ceiling, same shape as `run_golden_set`'s — `cost_of` turns `run_fn()`'s own return value into a `Money` amount (required together with `total_budget`, since `run_fn` is otherwise opaque); once the shared ledger runs out, remaining calls are skipped and `BenchmarkReport.budget_exhausted=True`. |
