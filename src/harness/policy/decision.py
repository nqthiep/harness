...
class DecisionLog:
    ...
    **Vì sao đồng bộ, không async.** `record`/`lookup` được gọi từ `_regate` của backend
    graph — code đồng bộ chạy trong node LangGraph — VÀ từ đường dispatch async của vòng
    lặp classic. Một API async sẽ buộc backend graph phải `asyncio.run()` giữa một node,
    nên chỗ này ghi bằng I/O file đồng bộ: một dòng, `flush`, `os.fsync`.
    """
    ...
    def record(self, d: Decision) -> Decision:
        ...
        if self._journal is not None:
            ...
            # này chứa dữ liệu nhạy cảm hơn transcript, nên nó không được sinh ra với
            # quyền đọc cho cả máy. Nói ra ở đây vì đây là đánh đổi, không phải sơ suất.
