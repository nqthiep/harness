    ...
    def _run_tools(self, state) -> dict:
        ...
        for p in state.get("_pending", []):
                    ...
                    async def _guarded(_call=_call, timeout=timeout):
                        async with asyncio.timeout(timeout):
                            return await once_if_unsafe(
                                ...
                                # `step` folded into the run-id half of the key, not
                                # `call_id` — matching the classic backend's own
                                # `dispatch.py::_invoke` key. A real provider's
                                # `tool_use` id is unique per call, but
                                # `FakeModel.tool_call()`'s convenience default
                                # (`call_id="c1"`) is not, and dozens of existing tests
                                # rely on that being inert across separate steps. Bug
                                # found on review: without `step` here, two DIFFERENT
                                # steps of the same thread issuing the same `call_id`
                                # collide on this persistent-store key, and the second
                                # call is wrongly treated as a replay of the first.
                                idempotency_key(f"{_run_id(state)}:"
                                                f"{state.get('step', 0)}", call["id"]),
            ...
            self._emit(state, EventKind.TOOL_FINISHED, step=state.get("step", 0),
                       tool=spec.name, call_id=call["id"],
