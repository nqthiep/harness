...
def _check_subagent_safety(toolset: ToolSet, parent_safety: str,
                           parent_approve: Any = None) -> None:
    ...
    **`safety` was the only axis this checked** (G-15, `design/review-architect.md`).
    `dispatch.py::_run_subagent` runs the CHILD's OWN `atry_run` with its OWN `approve=`,
    never the parent's — `run_child = child.with_(budget=...)` only ever replaces
    `budget`. And `PolicyEngine.resolve()` auto-ALLOWs a surviving ASK when `approve is
    None` and the call is neither `safety="strict"` nor `Effect.DANGER`. So a child built
    with no `approve=` at all, wrapped inside a parent that HAS a real human-approval
    callback, silently auto-approves exactly the write-effect-under-standard-safety
    decisions the parent's `approve=` exists to gate: delegating becomes a way to escape
    not just the safety LEVEL but the approver.

    Restriction-only, same framing as the rank check: a child needs an `approve=` of its
    OWN once its parent has one. It is not compared for equality, only for existence.

    **What this cannot check** (H-5, round 2, still holding in round 3). Unlike `safety`,
    `approve=` is not an ordered value to rank-compare — it is an arbitrary callback, and
    "exists" is the only property a construction-time check can cheaply verify.
    `Agent(approve=lambda call, **kw: True)` passes just as readily as a real
    human-in-the-loop callback. No static check can tell a rubber stamp from a real
    approver; that is inherent to any interface built around a caller-supplied function,
    not a gap specific to this one.

    `parent_approve` defaults to `None` — no parent approver, nothing to inherit — so a
    caller that only cares about the rank axis can still pass two arguments.
    """
    for spec in toolset:
        child = spec.subagent
        if child is None:
