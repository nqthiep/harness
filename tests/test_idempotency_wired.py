"""T-6.1 nối vào đường dispatch — `execute_once` cuối cùng có người gọi thật.
...
* `read`/`external` KHÔNG đi qua `execute_once`. Phát lại một `read` đã ghi sẽ trả về nội
  dung file TRƯỚC lúc chết — với agent code đó là lỗi đúng sai, không phải tính năng.
* Không có store thì mọi thứ chạy y như trước, không thêm một lần I/O nào.
"""
import asyncio
import os
import sys
...
from harness.idempotency import idempotency_key, once_if_unsafe
...
class HopDongCuaOnceIfUnsafe(unittest.TestCase):
    def test_khong_store_thi_goi_thang_khong_cham_vao_store(self):
        ran = []

        async def fn():
            ran.append(1); return "x"

        v, replayed = asyncio.run(once_if_unsafe(None, "k", fn, retryable=False))
        self.assertEqual((v, replayed, ran), ("x", False, [1]))

    def test_retryable_thi_goi_thang_du_co_store(self):
        store = InMemoryStore()
        ran = []
        ...
        async def twice():
            await once_if_unsafe(store, "k", fn, retryable=True)
            return await once_if_unsafe(store, "k", fn, retryable=True)

        v, replayed = asyncio.run(twice())
        self.assertEqual((v, replayed, ran), ("x", False, [1, 1]))
