...
def _sinks_only():
    """`CodeTools` minus its `danger` tools.

    These tests are about a WRITE SINK next to a camera — `private=True`, `allow_sinks`,
    and the confidentiality refusal between them. `CodeTools` also ships `run_tests` and
    `refresh_codebase_docs`, which became `danger` on the other branch (they execute
    project code), and `external` + `danger` is the lethal trifecta: it refuses the
    construction before any of these tests reach the thing they measure. Excluding them
    keeps each test measuring its own subject; the trifecta has its own tests.
    """
    return [t for t in CodeTools(root=".").tools() if t.effect is not Effect.DANGER]
...
class TheSinkRefusalThat(unittest.TestCase):
    ...
    def test_a_camera_next_to_write_tools_is_refused_by_default(self):
        with self.assertRaises(ConfigError) as caught:
            _agent(tools=_sinks_only()).with_profile(_profile())
    ...
    def test_private_true_resolves_it_by_mechanism(self):
        agent = _agent(tools=_sinks_only()).with_profile(
            _profile(private=True))
