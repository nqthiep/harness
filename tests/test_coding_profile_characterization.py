...
EXPECTED_EFFECTS = {
    ...
    "outline": "read", "read_source": "read", "refresh_codebase_docs": "danger",
    "run_command": "write", "run_shell": "write", "run_tests": "danger",
    ...
    def test_effect_classification_of_every_tool_is_pinned(self):
        for shell, findings in COMBOS:
            with self.subTest(shell=shell, findings=findings):
                ...
                # own assertion so a change reads as what it is. It DID change, and this
                # is what it was for: the line used to read `["read", "write"]` with the
                # claim that this profile "contributes no untrusted-content source and no
                # irreversible tool". Half of that is no longer true — `run_tests` and
                # `refresh_codebase_docs` became `danger` on the other branch (they
                # execute project code through the sandbox; a test suite can do anything
                # a shell can), and this test is how the merge noticed.
                #
                # What it costs the user, so nobody rediscovers it: `danger`'s
                # `decision_standard` is ASK, so under the default `safety="standard"`
                # every `run_tests` call now goes to an approver — the terminal prompt
                # this profile supplies when the caller brought none. Still no
                # `external` tool, so the lethal trifecta stays out of reach here.
                self.assertEqual(sorted(set(got.values())), ["danger", "read", "write"])
