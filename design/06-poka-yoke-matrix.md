...
| 2 | Cơ chế an toàn tồn tại nhưng không được bật | `agent_framework.security` không được `_harness/` import ([§09](../research/09-memory-context-multiagent-hitl.md) §16bis) | R-1: budget/taint/permission nằm trên đường đi bắt buộc; plugin chỉ cho policy. PLG-1: tập tác dụng phụ khi có plugin ⊆ khi không có | **3** | property test PLG-1 |
