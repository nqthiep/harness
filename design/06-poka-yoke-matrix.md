...
| 2 | A safety mechanism exists but isn't turned on | `agent_framework.security` never imported by `_harness/` ([§09](../research/09-memory-context-multiagent-hitl.md) §16bis) | R-1: budget/taint/permission on the mandatory path; plugins only for policy. PLUG-1 (K-13, renamed from the original `P-3`): the side-effect set with a plugin installed is a subset of without | **3** | `PluginRegistry.register` raises `ConfigError` at registration if a tool's effect exceeds the plugin's declared ceiling — a real tier-3 mechanism, no runtime path around it. `tests/test_m4.py::Plugins` (H-9, `design/review-architect-round3.md`: corrected from "a PLUG-1 property test") is four ordinary example-based unit tests (ceiling-exceeded, within-ceiling, version-mismatch, name-collision), not a property test iterating many plugin/tool combinations — the underlying mechanism holds, this column previously overstated the evidence backing it |
