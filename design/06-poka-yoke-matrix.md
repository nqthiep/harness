...
| `Sandbox.isolation` (G-7, design/review-architect.md) | **2** | the field makes the boundary *inspectable* (an audit event can now show whether a call ran `"none"`, `"process"`, or `"container"`) — it does not itself raise the ceiling. The shipped `InProcess` sandbox is still `"none"`; `Subprocess` is still only a process boundary, not a container/VM one. Same underlying gap as the `Workspace` row above, now just named on the record instead of silent |
