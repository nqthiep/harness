...
| **Nén context × cổng advisor** (phản hồi "kiểm tra tính toàn vẹn... sau merge", rồi "sửa đi") | Audit toàn vẹn một merge lớn (nén context thật + advisor gate, viết bởi hai phiên độc lập, không phiên nào biết phiên kia): `Runtime._tools_called()` (backend durable) quét lại `state["messages"]` mỗi lần, nhưng `Runtime._compact()` bỏ hẳn cặp `AIMessage`/`ToolMessage` của các bước cũ (thay bằng bia mộ không mang tên tool) — một `consult_advisor` gọi sớm trong hội thoại dài bị nén rơi ra ngoài thì `RequireBeforePolicy` DENY NHẦM `wipe` dù đã tư vấn thật. Xác nhận bằng repro trực tiếp trước khi vá (không chỉ đọc code): dựng `state["messages"]` tổng hợp, chạy qua `_manage()`/`_compact()` thật, thấy `tools_called` mất `consult_advisor` sau nén. Backend cổ điển kiểm riêng và xác nhận MIỄN NHIỄM: `Dispatcher.ran` là list nối-thêm cho cả đời một `Dispatcher`, không liên quan gì tới `msgs` (thứ `_manage_context()` sửa) | `state["tools_called_ever"]` (`lg/state.py`) — nối thêm ở `_run_tools`, không bao giờ bị nén xoá; `_tools_called()` trả UNION của field này với bản quét message cũ (không thay hẳn, để tương thích ngược với checkpoint cũ chưa có field) | ADR-067 (`docs/12-decision-logs.md`) | `test_advisor_gate.py::DurableGateSurvivesCompaction` (+3: nén thật xảy ra, tư vấn trước vẫn không bị chặn nhầm sau nén, không tư vấn vẫn bị chặn đúng sau nén) + mutation test xác nhận phụ thuộc đúng bản vá; 785 test pass toàn bộ |
