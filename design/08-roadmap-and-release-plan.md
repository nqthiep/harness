...
| **M8 — Observability** | 10% × gap | Envelope v1 (`schema_version`/`trace_id`/`tenant_id`), `Approval` là bản ghi đầy đủ, OTel exporter thật, cost-per-successful-task, event stream tiêu thụ được, `Session` resource | **Đang làm.** T-8.1 (envelope v1) ĐÃ XONG — 4 trường thêm vào `Event` (nối cuối, không chèn giữa — construction kiểu cũ vẫn chạy), `Agent`/`build_agent` nhận `tenant_id=`/`session_id=`, LangGraph's `session_id` = `thread_id` của chính nó. ADR-048. Khoá bằng `tests/test_m8_t81_envelope.py` (12 test, cả hai backend, mutation-tested). Kiểm lại T-8.2 trước khi làm thấy `Decision`/`DecisionLog` (`policy/decision.py`) ĐÃ có hầu hết trường `ApprovalRecord` đòi (`id`,`verdict`,`scope`,`actor`,`decided_at`,`expires_at`,`run_id`,`reason`) — xây từ S-11/S-29 sớm hơn trong phiên này, chỉ THIẾU `policy_version` và chỉ tồn tại ở backend LangGraph, không phải classic loop. T-8.2/T-8.3/T-8.4/T-8.5/T-8.6 chưa làm. |
