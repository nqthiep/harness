...
|---|---|---|---|
| **M6 — Reliability** | 12% × gap 2/5 | Idempotency key, cancellation đúng chuẩn (không nuốt `CancelledError`), retry theo effect class, failure injection | **Đang làm.** T-6.2 (cancellation) ĐÃ XONG (`tests/test_m6_t62_cancellation.py`). T-6.3 (retry theo effect class) ĐÃ XONG, **cả hai backend** — `dispatch.py::_invoke` (classic loop) và `lg/runtime.py::_run_tools` (LangGraph, port riêng vì hai backend không dùng chung code — thiếu port này ban đầu, tự bắt lại khi kiểm parity) đều đọc `EFFECT_PROFILES[...].retryable` (ADR-003) thật sự, retry `read`/`external` tới `MAX_ATTEMPTS`, `write`/`danger` luôn đúng một lần (ADR-042), khoá bằng `tests/test_m6_t63_retry.py` (8 test, mutation-tested cả hằng số lẫn nhánh logic, cả hai backend). T-6.1 (idempotency key) **contract xong, chưa gắn dây** — `idempotency.py::execute_once(store, key, fn) -> (result, was_replayed)` xây trên `Store` seam có sẵn (không seam mới), khoá bằng `tests/test_m6_t61_idempotency.py` (8 test + mutation), nhưng KHÔNG gắn vào `Agent`/`Dispatcher`/`Runtime` — quyết định có chủ đích, xem ADR-043 (`docs/12`): `run_id` mới mỗi `atry_run()` nên không có caller nào hôm nay tạo ra key trùng thật, gắn dây bây giờ là dead code chờ M9 cung cấp key từ Service API. T-6.4 (chaos) chưa. Phát hiện phụ khi kiểm parity T-6.3: **LangGraph backend không có timeout per-tool nào cả** (`spec.timeout_s` không được đọc ở đâu trong `lg/runtime.py`) — ghi ở `07-risks-and-open-issues.md` §1.5 (N-1), ngoài phạm vi T-6.3, chưa sửa. |
