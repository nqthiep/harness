...
|---|---|---|---|
| **M6 — Reliability** | 12% × gap 2/5 | Idempotency key, cancellation đúng chuẩn (không nuốt `CancelledError`), retry theo effect class, failure injection | **Đang làm.** T-6.2 (cancellation) ĐÃ XONG (`tests/test_m6_t62_cancellation.py`). T-6.3 (retry theo effect class) ĐÃ XONG — `dispatch.py::_invoke` giờ đọc `EFFECT_PROFILES[...].retryable` (ADR-003) thật sự, retry `read`/`external` tới `MAX_ATTEMPTS`, `write`/`danger` luôn đúng một lần (ADR-042), khoá bằng `tests/test_m6_t63_retry.py` (mutation-tested cả hằng số lẫn nhánh logic). T-6.1 (idempotency key)/T-6.4 (chaos) chưa. |
