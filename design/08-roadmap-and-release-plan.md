...
| (phản hồi người dùng sau N-10) | "compose 4 pattern thành middleware kiểu LangChain được không", rồi "chuẩn hoá tham số các hook lại" | `harness/middleware.py::Middleware` + `with_middleware()` — sugar dựng từ ba seam có sẵn (`ModelProvider`/tool callable/`Exporter`), KHÔNG phải seam thứ bảy. Mỗi hook nhận ĐÚNG MỘT object (`ModelCall`/`ToolInvocation`/`Event`), không còn tham số vị trí lệch nhau; danh tính (`run_id`/`call_id`/...) CỐ TÌNH không đưa vào — đã verify `contextvars` không sống sót qua `loop.run_in_executor()` của backend graph, nên để ngỏ thay vì đóng nửa vời trên một backend | `docs/03-public-api.md §3.6`, `docs/02-architecture.md §4` | `test_middleware.py` (15, gồm test khẳng định middleware không bypass được policy) |
