...
| **M9 — Integration** | 8% × gap 3/5 — **khoảng trống lớn nhất theo tự chấm** | MCP client làm tool boundary, Service API (`POST /v1/runs`...), canonical event adapter | **Đang làm.** T-9.1 (MCP client + `ServerIdentity`/`ServerLabel`) ĐÃ XONG — `src/harness/mcp.py`: `McpServerPolicy`, `classify_mcp_tool` (bốn luật M-1…M-4), `StdioMcpClient` (JSON-RPC 2.0 qua stdio thật, không SDK ngoài — 0 dependency mới), `McpBinding.check_for_rug_pull()` (fail-closed, khác đề xuất gốc — xem ADR-054). Đóng S-7…S-10, S-17 cùng lúc. `Scope.server`/`DecisionLog.lookup()` sửa lỗ hổng phụ phát hiện khi làm: trường `server` tồn tại từ trước nhưng chưa từng được so khớp (confused-deputy mở toang). `tests/test_m9_t91_mcp.py` (20 test), transport+rug-pull chạy qua tiến trình con thật. Service API và canonical event adapter — CHƯA làm. |
