...
| **M7 — Isolation** | 15% × gap — **nặng ký nhất theo trọng số nghiên cứu** | Workspace root, egress mặc định CHẶN (đảo `allowed_hosts=None` từ "cho tất cả" sang "chặn tất cả" — breaking change), seam `Sandbox`, secret không vào sandbox | **Đang làm.** T-7.1 (workspace root) ĐÃ XONG — `workspace.py::confine(root, path)`, cùng khuôn "từ chối, không escape" của IDL-44, khoá bằng `tests/test_m7_t71_workspace.py` (14 test đúng 4 kịch bản T-7.1 tự đặt: `..`, symlink, tuyệt đối, `..` mã hoá URL — cộng một lỗi pathlib thật tự bắt được khi viết, xem ADR-045). T-7.2/T-7.3/T-7.4 chưa. |
