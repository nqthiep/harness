...
| **M7 — Isolation** | 15% × gap — **nặng ký nhất theo trọng số nghiên cứu** | Workspace root, egress mặc định CHẶN (đảo `allowed_hosts=None` từ "cho tất cả" sang "chặn tất cả" — breaking change), seam `Sandbox`, secret không vào sandbox | **Đang làm.** T-7.1 (workspace root) ĐÃ XONG — `workspace.py::confine(root, path)`, khoá bằng `tests/test_m7_t71_workspace.py` (14 test, ADR-045). T-7.2 (egress mặc định chặn) ĐÃ XONG, **breaking change có chủ đích** — `allowed_hosts` mặc định đổi `None`→`()`, `None` tường minh vẫn là escape hatch (cùng khuôn S-20), ADR-046 (`docs/12`) giải thích vì sao KHÔNG làm một chu kỳ deprecation nhiều phiên bản (chưa có v1.0/người dùng ngoài để mà deprecate cho). Chạy full suite lộ ra 5 test cũ dựa vào default cũ để test THỨ KHÁC (taint) — sửa bằng `allowed_hosts=None` tường minh, không đổi ý nghĩa test. Khoá hành vi mới bằng `tests/test_m7_t72_egress_default.py` (6 test, cả hai backend, mutation-tested). T-7.3 (seam `Sandbox`)/T-7.4 (secret không vào sandbox) chưa. |
