...
| **M8 — Observability** | 10% × gap | Envelope v1 (`schema_version`/`trace_id`/`tenant_id`), `Approval` là bản ghi đầy đủ, OTel exporter thật, cost-per-successful-task, event stream tiêu thụ được, `Session` resource | **Đang làm.** T-8.1 (envelope v1) ĐÃ XONG — 4 trường thêm vào `Event`, `Agent`/`build_agent` nhận `tenant_id=`/`session_id=`, ADR-048, `tests/test_m8_t81_envelope.py` (12 test). T-8.2 (Approval là bản ghi) ĐÃ XONG — kiểm trước khi sửa thấy `Decision`/`DecisionLog` đã có hầu hết trường `ApprovalRecord` đòi (xây từ S-11/S-29), chỉ thiếu `policy_version` — thêm xong, `POLICY_ENGINE_VERSION` hằng số cùng khuôn `EVENT_SCHEMA_VERSION`, ADR-049. Hai tiêu chí "Failure"/"Test" T-8.2 tự đặt (hết hạn không dùng lại được; cùng approval không mở khoá lần chạy khác) hoá ra ĐÃ ĐÚNG SẴN trong `DecisionLog.lookup()` — khoá lại bằng test, không phải sửa logic. `tests/test_m8_t82_approval_record.py` (7 test, mutation-tested). Sẵn tiện sửa một lỗi tài liệu tìm thấy: `docs/04-interfaces.md` gọi `policy/base.py`'s class là `Decision` — tên thật trong code là `Ruling` (đổi tên từ trước để tránh va `policy/decision.py`'s `Decision`, đúng loại va chạm K-13 đang chờ dọn) — sửa lại tài liệu cho khớp code. T-8.3 (OTel)/T-8.4 (cost-per-success)/T-8.5 (event stream)/T-8.6 (`Session` resource) chưa làm. |
